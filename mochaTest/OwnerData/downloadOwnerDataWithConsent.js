...
describe('Download owner data as requester with given consent', function() {
    ...
    // download owner data by requester
    it('Should successfully download owner data by requester with given consent', function (done) {
        chai.request(Config.server).get('omr/api/v1/services/' + org2Service1.id + 
        '/owner_service/' + org1Service1.id + '/datatypes/' + org1Service1.datatypes[0].datatype_id + 
         '/downloadWithConsent?latest_only=false')
            .set('Accept', 'application/json')
