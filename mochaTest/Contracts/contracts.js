...
describe('Checking of Contract flow', function() {
    ...
    // download 4
    it('Should NOT successfully download owner data by requester', function (done) {
        chai.request(Config.server).get('omr/api/v1/contracts/' + contractId + 
        '/ownerdata/' + org1Service1.datatypes[0].datatype_id + '/downloadAsRequester')
            .set('Accept', 'application/json')
            .set('token', defaultOrg2Service1AdminToken)
    ...
    it('Should successfully get contract by requester', function (done) {
        chai.request(Config.server).get('omr/api/v1/contracts/' + contractId)
            ...
            .end(function(err, res){
                ...
                expect(res.body.contract_details).to.have.lengthOf(10);
                ...
                expect(res.body.contract_details[8].contract_detail_type).to.equal("download");
                expect(res.body.contract_details[9].contract_detail_type).to.equal("download");
                ...
                expect(res.body.num_download).to.equal(3);
                done();
            });
    });
    ...
    it('Should successfully get contracts by data owner filtered by status of contract', function (done) {
        chai.request(Config.server).get('omr/api/v1/contracts/owner/' + org1Service1.id + '/status/downloadReady')
            .set('Accept', 'application/json')
            .set('token', defaultOrg1Service1AdminToken)
            .end(function(err, res){
                expect(err).to.be.null;
    ...
    it('Should successfully get contracts by data owner filtered by status of contract', function (done) {
        chai.request(Config.server).get('omr/api/v1/contracts/owner/' + org1Service1.id + '/status/downloadDone')
            .set('Accept', 'application/json')
            .set('token', defaultOrg1Service1AdminToken)
            .end(function(err, res){
                expect(err).to.be.null;
                expect(res.body).to.be.an('array');
                expect(res.body).to.have.lengthOf(1);
                expect(res.body[0].contract_id).to.equal(contractId);
                expect(res.body[0].state).to.equal("downloadDone");
                done();
            });
    });

    it('Should successfully get contracts by data requester filtered by status of contract', function (done) {
        chai.request(Config.server).get('omr/api/v1/contracts/requester/' + org2Service1.id + '/status/*')
            .set('Accept', 'application/json')
            .set('token', defaultOrg2Service1AdminToken)
            .end(function(err, res){
                expect(err).to.be.null;
    ...
    it('Should successfully terminate contract by data owner', function (done) {
        let bodyRequest = {
            "contract_id": contractId,
            "terminated_by": org1Service1.id
          }
